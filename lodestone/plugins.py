...
class plugins:
    ...
    class schematic:
        ...
        async def builder(self, build, actions, status):
            layer = 1
            while len(build.actions) > 0:
                ...
                actions.sort(key=lambda a: distance_squared(Vec3(a['pos'].x + 0.5, a['pos'].y + 0.5, a['pos'].z + 0.5), self.bot.entity.position) if a['pos'] != self.bot.entity.position else float('inf'))
                ...
                if action["type"] == "place":
                    ...
                    await self.equip_item(item.id) # equip after pathfinder
                    ...
                    try:
                        sneak = interactable.index(refBlock.name) > 0
                    except ValueError:
                        sneak = False
                    try:
                        delta = faceAndRef.to.min(faceAndRef.ref)
                    except:
                        pass
                    if sneak: 
        ...
        def start(self, file=""):
            ...
            def build_scematic(bot, file):
                with self.console.status("[bold]Loading schematic...\n") as status:
                    # filename = "../examples/schematics/smallhouse.schem"

                    # with open(filename, 'rb') as f:
                    #   schematic_data = f.read()
                    
                    
                    # file = '../examples/schematics/simple-house-2.schem'
                    
                    # if ".schematic" in file:
                    #     loop = asyncio.get_event_loop() 
                    #     loop.run_until_complete(convert(file))
                    #     loop.close()
                    #     file = str(file).replace(".schematic",".schem")
